...
if ($type == "everything")
{
	echo"
	<script>
		window.location.assign(\"http://howlix.com/shred2/main.php\");
	</script>
	";
}
...
?>
        ...
        <div class="col-md-9" id="mainCol">
			...
			<input type="submit" value="submit">
                        ...
                        The Latest Shreds Sorted by <?php echo $type; ?> 
                </h2>

<?php
//sqlStart();
	//sqlQuery("SELECT * FROM $main_table WHERE type='$type' AND ORDER BY ID DESC");

	$sql = "SELECT * FROM $main_table WHERE type='$type' ORDER BY ID DESC";
		...
		while($row = mysql_fetch_array($result))
			...
			echo"
				...
				<a href=\"#\"><img src=\"http://graph.facebook.com/" . $uid . "/picture\" class=\"fl mr15 pic\" /></a>
                        	...
                        	<p><small>added by " . $ushred_name . " at " . $date . " &bull; <a href=\" " . $url . " \" target=\"_new\"> " . $url . " </a> &bull; <a href=\"http://howlix.com/shred2/folder.php?folder=" . $folder . "&fid=" . $fid . "&squawk=" . $Sfid . "\"> " . $folder . "</a> &bull; " . $type . "</small></p>
                        	</div>
                        	</div>
                        	<!-- End Shred -->
                        	<hr />
